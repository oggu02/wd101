...
const dispEntries = () => {
    ...
    const tableEntries = entries.map((entry) => {
        ...
        const atc = `<td class="bor">${entry.atc}</td>`;

        const row = `<tr>${name} ${email} ${password} ${dateOfBirth} ${atc}</tr>`;
        return row;
    }).join("\n");
...
const formSubmit = (event) => {
    ...
    const atc = document.getElementById("atc").checked;

    const entry = {
        name, email, password, dob: dateOfBirth, atc
    }
